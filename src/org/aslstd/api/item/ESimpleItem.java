...
import org.aslstd.api.rarity.RandomRarity;
...
public final class ESimpleItem extends ItemStack implements EItem {
	...
	public ESimpleItem setRandomAttributes() {
		for (final Entry<String,RandomVal> entry : random.entrySet()) {
			...
			boolean single = stat.getType() == AttrType.RANGE ? false : true;
			value = value.replace("%", "");
			split = value.split("-");
	...
	public ESimpleItem setAttributes(List<Entry<String,String>> attrSet) {
		...
		for (final Entry<String,String> key: attrSet) {
			...
			final boolean percent = NumUtil.isPercent(value);
			boolean single = stat.getType() != AttrType.RANGE;
			value = value.replace("%", "");
			split = value.split("-");
	...
	public void changeAttr(final AttrBase attr, String value) {
		...
		if (attr.getType() == AttrType.RANGE) {
	...
	public void removeAttr(final AttrBase stat, boolean random) {
		if (random) {
			if (settings.hasKey("random-attributes." + stat.getKey().toLowerCase())) {
				...
				getFile().set(getKey() + ".random-attributes." + stat.getKey().toLowerCase(), null);
			}
		} else
			if (settings.hasKey("attributes." + stat.getKey().toLowerCase())) {
				settings.remove("attributes." + stat.getKey().toLowerCase());
				getFile().set(getKey() + ".attributes." + stat.getKey().toLowerCase(), null);
			}
